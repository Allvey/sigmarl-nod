...
若车辆 \(i\) 和 \(j\) 的预测路径可能在未来时间窗内占用同一冲突区域，则建立边：
...
预测残差只是证据网络的一类物理输入，不是意见动力学的核心组成部分。
...
\[
V_\omega\left(x_t,\operatorname{sg}[\mathbf z_t]\right),
\]

其中 \(\operatorname{sg}\) 表示停止梯度。Critic 可以读取意见状态以改善价值估计，但价值损失不能更新证据网络。
