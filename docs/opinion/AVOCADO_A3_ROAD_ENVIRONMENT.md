...
A4已经把冻结的Base-MAPPO名义动作接入固定A3安全链，并保留A3作为可复现对照组；
实现与运行方式见[A4动作级耦合](AVOCADO_A4_MARL_ACTION_COUPLING.md)。下一阶段A5/A6
继续保留A3实时计算的注意力 \(A\)、启发式邻车合作估计 \(y^H\) 和原非线性意见递推，
由MARL只学习有界修正 \(\Delta y^{RL}\)；零修正必须逐步退化为A4。完整定义见
